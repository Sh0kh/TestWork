      ...
      <div className='w-full p-[24px] sm:p-[0px]'>
        <form>
          <label htmlFor="search" className='flex  gap-[10px] '>
            ...
            placeholder='Search for TV series' className='border-b-[1px] border-b-[#10141E] pb-[15px] pl-[16px] bg-transparent outline-none text-white w-9/12	' type="text" id='search' />
          </label>
        </form>
      </div>
      <section className="Section px-[24px] sm:px-[0px] mt-[40px] ">
          <div className="container">
            <h1 className="text-2xl sm:text-4xl	 text-white">TV Series</h1>
            <div className="Wrapper grid grid-cols-4 mt-[32px] pr-[20px]">
              {isTV
                .filter((item) => item.type === "TV Series" && item.name.toLowerCase().includes(searchTerm.toLowerCase()))
                .map((item) => (
                  <div
